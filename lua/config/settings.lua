...
vim.api.nvim_create_autocmd(
  { "FileType" },
  { pattern = { "rust" }, command = "setlocal tabstop=2 softtabstop=2 shiftwidth=2 expandtab" }
)
vim.api.nvim_create_autocmd(
  { "FileType" },
  { pattern = { "go" }, command = "setlocal tabstop=4 softtabstop=4 shiftwidth=4 expandtab" }
)
do
  local options = {
    expandtab = true,
